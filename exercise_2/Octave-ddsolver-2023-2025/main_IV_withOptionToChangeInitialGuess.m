...
device.doping.NA = 5E23; % [m^3]  5E23
...
disp(['Number of voltage points computed :',num2str(length(voltage_ramp))]);
...
% we ramp the IV curve in a split fashion
%   we define a high voltage (not the highest) as starting point
%   then, one loop checks the current for decreasing the voltage from there
%   and a second look checks the current for increasing the voltage from there

voltage_forscan_start = 1.0;
voltage_idx_forscan_start = max(find(voltage_ramp < voltage_forscan_start)); 

% first loop scans downward in voltage
for bias_voltage_num = voltage_idx_forscan_start:-1:1  
    ...
    % decide whether previous calculations shall serve as initial guess
    if (bias_voltage_num < voltage_idx_forscan_start)
      ...
      disp(['Use initial guess from prev calc for voltage = ',num2str(voltage_ramp(bias_voltage_num))]);
    else
      itercontrol.prev_guess = false;
      [current_ramp(bias_voltage_num),qprofile, it, res] = ...
...
% second loop scans upward in voltage
for bias_voltage_num = voltage_idx_forscan_start+1:1:length(voltage_ramp)  
    ...
    % decide whether previous calculations shall serve as initial guess
    if (bias_voltage_num > voltage_idx_forscan_start+1)
